...
export default async function AdminTeamPage({ searchParams }: { searchParams: Promise<{ assigned?: string; revoked?: string; error?: string }> }) {
  ...
  return <main className="admin-detail"><header><div><p className="kicker">ACCESS CONTROL</p><h1>Administrative team and privileges</h1><p>Grant several operational privileges to one registered PAC-SM email in a single action.</p>{query.assigned && <p className="success-note">{query.assigned} privilege{query.assigned === "1" ? "" : "s"} assigned.</p>}{query.revoked && <p className="success-note">Privilege revoked.</p>}{query.error === "roles" && <p className="form-error">Select at least one administrative privilege.</p>}{query.error === "not_found" && <p className="form-error">No registered PAC-SM account was found for that email.</p>}{query.error === "superadmin" && <p className="form-error">Super Administrator privileges cannot be changed from Admin Team.</p>}{query.error === "scope" && <p className="form-error">The Appwrite runtime key cannot manage users. Enable users.read and users.write on the single Coolify APPWRITE_API_KEY.</p>}{query.error === "assignment" && <p className="form-error">Appwrite could not save one or more selected privileges. Please retry or check the application log for the exact backend response.</p>}</div><Link href="/admin">Admin overview</Link></header><section className="admin-role-summary"><article><span>AVAILABLE PRIVILEGES</span><strong>{ADMIN_ROLES.length}</strong></article><article><span>ACTIVE ASSIGNMENTS</span><strong>{activeCount}</strong></article><article><span>CONTROL MODEL</span><strong>Multiple per account</strong></article></section><section className="catalogue-create admin-role-create"><h2>Assign multiple privileges</h2><p>Enter one registered email, then select every responsibility this administrator should hold.</p><form action="/api/admin/team" method="post"><label className="admin-role-email">User email<input name="email" type="email" autoComplete="email" required /></label><fieldset><legend>Administrative privileges</legend><div className="admin-role-options">{ADMIN_ROLES.map(role => <label key={role}><input type="checkbox" name="roles" value={role}/><span><strong>{labels[role]}</strong><small>{role.replaceAll("_", " ")}</small></span></label>)}</div></fieldset><button>Grant selected privileges</button></form><p>Privileges are cumulative and can be revoked individually below. Super Administrator access cannot be granted or revoked from this screen.</p></section><section className="review-table"><div className="review-row review-head"><span>Email</span><span>Privilege</span><span>Status</span><span>Action</span></div>{members.documents.map(member => <div className="review-row" key={member.$id}><span>{String(member.email)}</span><strong>{labels[String(member.role)] ?? String(member.role)}</strong><span>{String(member.status)}</span>{member.status === "active" ? <form action={`/api/admin/team/${member.$id}/revoke`} method="post"><button>Revoke this privilege</button></form> : <span>{member.revokedAt ? new Date(String(member.revokedAt)).toLocaleDateString("en-GB") : "—"}</span>}</div>)}</section></main>;
}
