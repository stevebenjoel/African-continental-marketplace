...
export async function POST(request: Request) {
  ...
  if (!user) return Response.redirect(new URL("/login?returnTo=/vendor/register", request.url), 303);
  if (await findVendorByOwner(user.$id)) return Response.redirect(new URL("/vendor/register", request.url), 303);
  ...
  if (required.some((key) => !text(form, key))) return Response.redirect(new URL("/vendor/register?error=Complete+all+required+fields", request.url), 303);
  const files = ["registration_certificate", "tax_document", "owner_identity"].map((type) => ({ type, file: form.get(type) }));
  if (files.some(({ file }) => !(file instanceof File) || file.size < 1 || file.size > 20 * 1024 * 1024 || !["application/pdf", "image/jpeg", "image/png"].includes(file.type))) return Response.redirect(new URL("/vendor/register?error=Upload+valid+PDF,+JPG+or+PNG+documents+under+20MB", request.url), 303);
  try {
    ...
    return Response.redirect(new URL("/vendor/register?submitted=1", request.url), 303);
  } catch (error) {
    console.error("Vendor application failed", error);
    return Response.redirect(new URL("/vendor/register?error=Application+could+not+be+submitted", request.url), 303);
  }
}
