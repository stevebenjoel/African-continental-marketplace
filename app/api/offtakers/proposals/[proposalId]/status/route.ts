...
export async function POST(request: NextRequest, context: { params: Promise<{ proposalId: string }> }) { try { assertSameOrigin(request); } catch { return new Response("Forbidden", { status: 403 }); } const user = await getCurrentAppwriteUser(); if (!user) return new Response("Unauthorized", { status: 401 }); const { proposalId } = await context.params; const form = await request.formData(); await updateProposalStatus(user.$id, proposalId, String(form.get("status") ?? "")); return NextResponse.redirect(publicAppUrl("/offtakers"), 303); }
