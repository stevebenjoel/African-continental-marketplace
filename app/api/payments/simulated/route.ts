...
export async function POST(request: Request) {
  ...
  try { await captureSimulatedPayment(user.$id, orderId); return Response.redirect(new URL(`/orders/${orderId}?paid=1`, request.url), 303); }
  catch (error) { console.error("Simulated payment failed", error); return Response.redirect(new URL(`/orders/${orderId}?paymentError=1`, request.url), 303); }
}
