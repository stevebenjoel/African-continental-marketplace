...
import { sendRegistrationWelcome } from "@/src/modules/notifications/server/account-email";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const user = await account.create({ userId: ID.unique(), ...input });
    ...
    try {
      await sendRegistrationWelcome({ userId: user.$id, email: input.email, name: input.name });
    } catch (error) {
