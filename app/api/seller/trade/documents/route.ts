...
export async function POST(request: Request) {
  ...
  if (!(file instanceof File) || file.size < 1 || file.size > 20 * 1024 * 1024 || !["application/pdf", "image/jpeg", "image/png"].includes(file.type)) return Response.redirect(new URL("/seller/trade?error=document", request.url), 303);
  try { const fileId = ID.unique(), bucketId = env().APPWRITE_KYC_BUCKET_ID; await createAppwriteAdminClient("storage").storage.createFile({ bucketId, fileId, file: InputFile.fromBuffer(Buffer.from(await file.arrayBuffer()), file.name), permissions: [] }); await addTradeDocument({ vendorId: vendor.$id, ownerUserId: user.$id, documentType: String(form.get("documentType") ?? "supporting_document"), referenceNumber: String(form.get("referenceNumber") ?? ""), bucketId, fileId, filename: file.name, mimeType: file.type, sizeBytes: file.size, status: "submitted", uploadedAt: new Date().toISOString() }); return Response.redirect(new URL("/seller/trade?document=1", request.url), 303); }
  catch (error) { console.error("Trade document upload failed", error); return Response.redirect(new URL("/seller/trade?error=document", request.url), 303); }
}
