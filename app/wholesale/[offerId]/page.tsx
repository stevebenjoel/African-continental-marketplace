    ...
    <section className="wholesale-transaction-zone">
      ...
      <div className="wholesale-action-panel">
        ...
        {!user ? <div className="wholesale-gate"><h3>Sign in as a business buyer</h3><p>Your selected product will remain the destination after sign-in.</p><Link href={loginHref}>Sign in to continue →</Link></div> : !buyer ? <div className="wholesale-gate"><h3>Business registration required</h3><p>Register your organization and return after PAC-SM approval.</p><Link href="/wholesale#buyer-registration">Register as a business buyer →</Link></div> : !approvedBuyer ? <div className="wholesale-gate"><h3>Review in progress</h3><p>Your business buyer status is <strong>{String(buyer.status).replaceAll("_", " ")}</strong>. Transaction controls unlock after approval.</p></div> : <div className="wholesale-action-options">
          ...
          {settings.enabled === "true" && <section className="white-label-negotiation" id="white-label"><div><span>OPTION 3 · PRIVATE LABEL</span><h3>Request white-label production</h3><p>Ask the seller to manufacture this product under your brand. Price, volume and customization remain negotiable until both sides accept.</p></div><form action="/api/wholesale/negotiations" method="post"><input type="hidden" name="offerId" value={offerId}/><input type="hidden" name="negotiationType" value="white_label"/><div className="negotiation-form-grid"><label>Brand or business name<input name="brandingName" required maxLength={180}/></label><label>Quantity<input name="quantity" type="number" min={Number(settings.minimumQuantity ?? offer.minimumOrderQuantity)} defaultValue={Number(settings.minimumQuantity ?? offer.minimumOrderQuantity)} required/></label><label>Proposed unit price ({String(offer.currency)})<input name="unitPrice" type="number" min="0.01" step="0.01" defaultValue={(basePrice / 100).toFixed(2)} required/></label><label>Delivery country code<input name="deliveryCountryCode" minLength={2} maxLength={2} placeholder="NG" required/></label><label>Required delivery date<input name="requestedDeliveryAt" type="date"/></label><label className="wide">Branding and packaging requirements<textarea name="customizationBrief" minLength={20} maxLength={3000} required placeholder="Describe logo placement, labels, packaging, colours, sizes and certification requirements."/></label><label className="wide">Commercial message<textarea name="message" maxLength={2000} placeholder="Add payment, sample, production or delivery terms for the seller."/></label></div><button>Send white-label request to seller →</button></form></section>}
        </div>}
      </div>
    </section>
