...
Phase 2 adds a protected, read-only Global Product Discovery workspace for CJ categories, bounded search, product inspection, variants, indicative USD supplier costs, inventory and warehouses. Discovery never publishes or imports a product. Remote media URLs are restricted to CJ-controlled HTTPS hosts. Phase 3 remains the next gate for selective import into the Diplomats Stores catalogue.
