...
export function ProductImage({ media, productName, className = "product-media" }: { media: MediaLike; productName: string; className?: string }) {
  ...
  return <div className={className}><Image src={`/api/catalogue/media/${media.$id}`} alt={String(media.altText || productName)} fill sizes="(max-width: 520px) 100vw, (max-width: 900px) 50vw, 25vw" unoptimized /></div>;
}
